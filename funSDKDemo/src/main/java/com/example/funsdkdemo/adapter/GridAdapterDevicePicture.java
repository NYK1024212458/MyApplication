...
public class GridAdapterDevicePicture extends BaseAdapter implements StickyGridHeadersBaseAdapter, OnFunDeviceFileListener {
    ...
    public interface OnGridDevicePictureListener {
		public void onGridDevicePictureDayDisplay(int position, SameDayPicInfo picInfo);
    ...
    public DevCmdOPRemoveFileJP getRemovingFiles() {
        ...
        for (Map.Entry<Integer, Boolean> entry : mIsSelected.entrySet()) {
            if (entry.getValue()) {
                ...
                FunFileData data = getFileData(position);
                String fileName = data.getFileName();
                if (null == fileName) {
                    Toast.makeText(mContext, "error fileName:" + position,
                ...
                int isDir = (data.getFileType() == SDKCONST.PicFileType.PIC_BURST_SHOOT || data
                        ...
                        : 0;
                cmdOPRemoveFileJP.addFileNameInfo(position, SDKCONST.StreamType.Main, isDir, fileName);
                mLruCache.remove(fileName);
                removeFileData(position);
            }
        }
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if ( oldPosition != position ) {
			...
			if ( oldPosition >= 0 ) {
				if ( mDispPosition.contains((Integer)oldPosition) ) {
					mDispPosition.remove((Integer)oldPosition);
                }
			}
			
			if ( !mDispPosition.contains((Integer)position) ) {
				mDispPosition.add((Integer)position);
            }
	...
	private class FilePosition {
		...
		public boolean equals(Object o) {
			...
			if ( fp.day_index == this.day_index
