...
import com.lib.funsdk.support.config.ChannelTitleDotSet;
...
public class ActivityGuideDeviceSetupCamera extends ActivityDemo
	...
	private void refreshCameraConfig() {
		if (mFunDevice instanceof FunDeviceSocket) {
			...
			if (null != fVideoOsd) {
				// 水印开关
				mBtnSwitchOSD.setSelected(fVideoOsd.getEnable());
			}
...
//				setOSDName(avEnc.getChannelTitle());
	...
	private void trySaveCameraConfig() {
		...
		if (mFunDevice instanceof FunDeviceSocket) {
			...
			if (null != fVideoOsd) {
				// 水印开关
				if (mBtnSwitchOSD.isSelected() != fVideoOsd.getEnable()) {
					fVideoOsd.setEnable(mBtnSwitchOSD.isSelected());
					beFVideoOsdLogoChanged = true;
				}
			}
