...
public class ActivityGuideDeviceSetupEncode extends ActivityDemo
	...
	int GetResolutionSize(int nResIndex) {
		if (nResIndex >= 0 && nResIndex <= SDK_CAPTURE_SIZE_t.SDK_CAPTURE_SIZE_NR) {
	...
	int GetResMark(int nLastAbility, int nRate, int nSupportResMark) {
		int nRetMark = 0;
		int nGetRes = 0;
		for (int i = 0; i <= SDK_CAPTURE_SIZE_t.SDK_CAPTURE_SIZE_NR; ++i) {
	...
	public int N_RESOLUTION_COUNT = SDK_CAPTURE_SIZE_t.SDK_CAPTURE_SIZE_NR;
	...
	String GetResFromIndex(int index) {
		if (index >= 0 && index <= SDK_CAPTURE_SIZE_t.SDK_CAPTURE_SIZE_NR) {
